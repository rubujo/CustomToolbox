...
- v1.2.3
  1. 更新第三方函式庫。
  2. 更新預設的使用者代理字串。（Google Chrome v136）
  3. 配合功能異動更新 MANUAL.md。
  4. 調整快速鍵，將原先 T 鍵的功能移至 Y 鍵，將 T 鍵的功能變更成開／關彈出視窗顯示在最上層。
  5. 移除 Bilibili API 的相關實做，改為使用 Playwright 重寫 Bilibili 短片清單產生器功能。
  6. 移除 Discord 豐富狀態。
  7. 因為部分引用資源的授權變更，所以移除相關程式碼，例如：自動歌詞相關功能。
  8. 配合 Whisper.net 函式庫的異動調整程式。
  9. 更新 README.md 的內容。
  10. 更新語系檔案，移除不再需要的語系字串。
- v1.2.2
  1. 更新第三方函式庫。
